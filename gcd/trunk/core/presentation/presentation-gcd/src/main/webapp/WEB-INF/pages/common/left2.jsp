							...
							<c:if test="${categoryDTO.categoryId==hash.key}">
								<ul id="subcategory">
									<c:forEach var="subCategories" items="${hash.value}">
										...
										<li><a href="http://neworleans.getcitydealz.com/dealsbysubcategory/${categoryName4}-${categoryDTO.categoryId}/${subCtgName4}-${subCategories.subCategoryId}">${subCategories.subCategoryName}(${subCategories.counts})</a></li>
									</c:forEach>
								</ul>
							</c:if>
						</c:forEach></li>
