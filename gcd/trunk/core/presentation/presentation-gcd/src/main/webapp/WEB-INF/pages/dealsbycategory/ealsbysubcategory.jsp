...
<title><c:if test="${cityDTO!=null}">${subCategoryName} - Daily Deals in ${cityDTO.cityName}, ${cityDTO.stateName} | Events, Promos & More in ${cityDTO.cityName} | Get City Dealz</c:if> <c:if test="${cityDTO==null}">Find the Best Deals, Bargains, Events & Promos in Your City | Get City Dealz</c:if></em></title>
			...
			<div class="main-content">
				...
				<h3 class="msg">
					<c:if test="${dealzs!=null}">
						<em>The Best ${subCategoryName} Daily Deals in New Orleans</em>
					</c:if>
					<c:if test="${dealzs==null}">Currently there are no deals for the ${subCategoryName} but please stop back soon and check for new ones or subscribe  to get notified when new deals are announced for the ${subCategoryName}</c:if>
				</h3>
						...
						<div id="deal_content">
							...
							<div class="deal-content">
								<table width="100%" border="0" cellpadding="0" cellspacing="0">
									<tbody>
										<tr>
											<td colspan="3"><span itemprop="seller"><strong><a class="merchant" id="merchant_name_link" style="color: #28708D;" href="http://${listVar.cityName}<spring:message code="common.city.domain"></spring:message>/merchant/${listVar.urlMerchantName}-${listVar.merchantId}"><c:out
										...
										<tr>
											...
											onclick="shareOnFB('${listVar.urlMerchantName}/deal/${listVar.urlTitle}-${listVar.dealId}','<c:out value="${listVar.imagePath}" />','<c:out value="${title_c}" />','<c:out value="${description2}" />','redirect');" target="_blank"><img
												src="<c:url value='/resources/images/facebook-share-this-deal-ic1.png'/>" width="80px" height="30px"></a> --%>
