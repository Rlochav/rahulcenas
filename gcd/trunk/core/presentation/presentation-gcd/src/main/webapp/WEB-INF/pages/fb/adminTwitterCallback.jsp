		...
		<div class="cent  no-right">
			<div class="main-content">
				...
				<div id="dashboard-area">
					<div id="title">${screenName}: Twitter Auth Details</div>
					<br>
					<div align="right">
					...
					<form:form id="twitterForm" name="twitterForm" class="formStyle">
						<h3 class="msg">
						...
						<div class="profile">
							<table cellspacing="0" cellpadding="0" style="width: 50%;">
								...
								<tr>
									<td>Screen Name: ${screenName}</td>
								</tr>
								<%-- <tr>
											<td>Token: ${token}</td>
										</tr>
										<td>Token Secret: ${tokenSecret}</td> --%>
								</tr>
								<tr>
									<td><input type="button" onclick="saveTwitterDetails();" name="continue" id="continue" value="Continue"></td>
								</tr>
							</table>
						</div>
					</form:form>
...
$(function () {
	...
	try{
		var a = '<optgroup label="Personal Profile"><option>${facebook.name}</option></optgroup>';
		$("#selectTag").append(a);
	}catch (e) {
		alert(e);
	}
...
function saveTwitterDetails(){
	document.twitterForm.action="saveTwitterDetails?userId=${userId}&screenName=${screenName}&token=${token}&tokenSecret=${tokenSecret}";
	document.twitterForm.submit();
}
</script>
</html>