...
<title><c:if test="${cityDTO!=null}">Local ${nDto.neighborhoodName} - Deals in ${cityDTO.cityName}, ${cityDTO.stateName} | Best Bargains in ${cityDTO.cityName} | Get City Dealz</c:if> <c:if test="${cityDTO==null}">Find the Best Deals, Bargains, Events & Promos in Your City | Get City Dealz</c:if></title>
	...
	<div id="wrapper">
		...
		<div class="cent center-both no-right">
			...
			<div class="main-content">
				...
				<div id="title"></div>
				<h3 class="msg">
					<em><c:if test="${dealzs!=null}">
							<em>The Best ${nDto.neighborhoodName} Daily Deals ${cityName}</em>
						</c:if> <c:if test="${dealzs==null}">Currently there are no deals for the ${nDto.neighborhoodName} but please stop back soon and check for new ones or subscribe  to get notified when new deals are announced for the ${nDto.neighborhoodName}</c:if></em>
				</h3>
