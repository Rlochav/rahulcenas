...
@Controller
public class CredentialResetController extends AppWebHandler {
	/**
	 * 
	 *//*
	...
	@RequestMapping(value = "/admin/resetUserName")
	public String remind(UserMaster userMaster, BindingResult bindingResult, ModelMap model, RedirectAttributes redirectAttributes) throws BaseException {
		...
		try {
			...
			MerchantDTO dto = merchantService.getMerchantDetailByEmailId(userMaster.getEmailId());
			if (dto == null)
				dto = merchantService.getAdminDetailsByEmailId(userMaster.getEmailId());
			if (dto == null) {
				redirectAttributes.addFlashAttribute("query", getRequestParam("query"));
			...
			if (holderMap.size() > 0) {
				EmailUtility.postMailType2("01", mEmailId, null, null, subject, template, holderMap,2);
	...
	@RequestMapping(value = "/admin/resetPasword")
