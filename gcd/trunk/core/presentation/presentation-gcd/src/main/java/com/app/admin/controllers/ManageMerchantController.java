...
public class ManageMerchantController extends AppWebHandler {
	...
	public ModelAndView manageMerchantDisplay() {
		...
		try {
			...
			String order = getRequestParam("order");
			if (isEmpty(order))
				order = "ASC";

			MerchantDTO merchantDTO = merchantService.getAllMerchantList(userInfo, start, order);
			if (merchantDTO != null) {
	...
	public ModelAndView infoMerchantDisplay(ModelMap modelMap) {
		UserInfo userInfo = getSessionAttribute(AppWebConstant.SESSION_ATTR_USER_INFO) != null ? (UserInfo) getSessionAttribute(AppWebConstant.SESSION_ATTR_USER_INFO)
				: null;
		...
		try {
			...
			UserInfo merUserInfo = new UserInfo();
			merUserInfo.setUserIndex(IntUtil.getInteger(getRequestParam("merchantId")));
			LocationDTO locationDTOs = merchantService.getAdditionLocationByMerchant(merUserInfo);
	...
	public String socialConnectionsDisplay(ModelMap modelMap) {
		try {
			...
			modelMap.put("merchantDto", merchantService.getMerchantDetailById(getRequestParam("merchantId"), null));
		} catch (Exception e) {
			e.printStackTrace();
		}
			...
			for (MultipartFile multipartFile : files) {
				...
				try {
					...
					try {
						/*
						 * DocumentDTO docDto =
						 * getCityDealz.getDocumentDetailsByMerchantId
