...
public class FileUploadFilter implements Filter {
		...
		if ("FILEUPLOAD".equalsIgnoreCase(request.getParameter("command"))) {
			...
			if ((request instanceof HttpServletRequest) && (response instanceof HttpServletResponse)) {
				...
				if ((contentLength != null && Integer.parseInt(contentLength) == 0)) {
					...
					setSessionCookie((HttpServletResponse) response, httpRequest);
					return;
				}
			}
		}
	...
	private String getSessionCookieName() {
		return config.getInitParameter(USER_SESSION_COOKIE_NAME) == null ? JSID_PARAM_NAME : config.getInitParameter(USER_SESSION_COOKIE_NAME);
	}

	private String getSessionParameterName() {
		return config.getInitParameter(USER_SESSION_PATH_PARAMETER_NAME) == null ? JSESSIONID : config.getInitParameter(USER_SESSION_PATH_PARAMETER_NAME);
	}
}
