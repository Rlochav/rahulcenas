...
public class CitiesController extends AppWebHandler {
	...
	public ModelAndView editCitydisplay(ModelMap modelMap) {
		ModelAndView mav = new ModelAndView("admin/editcity");
		String btnName = "Add New";
		String title = "Add A New City";
		...
		try {
			if (getRequestParam("cityId") != null && !getRequestParam("cityId").trim().isEmpty() && getRequestParam("stateId") != null
				...
				cityDTO = getCityDealz.getCityDetailsById(getRequestParam("cityId"), getRequestParam("stateId"));
				btnName = "Update";
				
				if (cityDTO != null && getRequestParam("cityId") != null) {
					title = "Edit " + getCityDealz.getCityText(getRequestParam("cityId")) + " City";
				}

			}
			
			Map<String, String> timeZoneMap = getCityDealz.getTimeZones();
			modelMap.put("cityName", title);
	...
	public CityDTO saveCityDetails() {
		...
		try {
			...
			if (getRequestParam("timeZoneId") != null && !getRequestParam("timeZoneId").trim().isEmpty())
				cityDTO.setTimeZoneId(getRequestParam("timeZoneId"));
