...
public class ConnectorServlet extends HttpServlet {
	 ...
	 *             when error occurs.
	 */
	private void getResponse(final HttpServletRequest request, final HttpServletResponse response, final boolean post) throws ServletException {
		...
		try {
			...
			if (Boolean.valueOf(getServletConfig().getInitParameter("debug")) && e.getException() != null) {
	 ...
	 */
	private void handleError(final ConnectorException e, final IConfiguration configuration, final HttpServletRequest request,
			final HttpServletResponse response, final String currentCommand) throws ServletException {
		try {
	...
	public void init() throws ServletException {
		ServletContextFactory sc = new ServletContextFactory();
		Class<?> c = sc.getClass();

		try {
			Field f = c.getDeclaredField("servletContext");
			f.setAccessible(true);
			f.set(ServletContext.class, getServletContext());
		} catch (Exception e1) {
		...
		try {
			...
			if (className != null) {
				...
				if (clazz.getConstructor(ServletConfig.class) != null) {
					configuration = (IConfiguration) clazz.getConstructor(ServletConfig.class).newInstance(getServletConfig());
	...
	private enum CommandHandlerEnum {
		 ...
		 *             when error occurs
		 */
		private void execute(final HttpServletRequest request, final HttpServletResponse response, final IConfiguration configuration, final ServletContext sc,
