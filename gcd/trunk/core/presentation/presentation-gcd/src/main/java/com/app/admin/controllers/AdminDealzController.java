...
public class AdminDealzController extends AppWebHandler {
	...
	public ModelAndView editModeDisplay(ModelMap modelMap) {
		...
		try {
			mav.addObject("merchantDto", merchantService.getMerchantDetailById(getRequestParam("merchantId"), null));
			if (getRequestParam("dealzId") != null && !getRequestParam("dealzId").trim().isEmpty()) {
				dealzDTO = merchantService.getDealzDetailById(userInfo, getRequestParam("dealzId"), getRequestParam("dealType"));
				Map<String, String> timeZoneMap = getCityDealz.getTimeZones();
				modelMap.put("timeZoneMap", timeZoneMap);
				CityDTO cityDtos = getCityDealz.getCityDetails(dealzDTO.getCity());

				dealzDTO.setTimeZoneId(cityDtos.getTimeZoneId());

				mav.addObject("dealType", getRequestParam("dealType"));
				mav.addObject("merchantDealzForm", dealzDTO);
	...
	public String saveMerchantDealz(@ModelAttribute("uploadForm") com.app.mvc.merchant.DealzDTO uploadForm, ModelMap map, RedirectAttributes redirectAttributes) {
		UserInfo userInfo = getSessionAttribute(AppWebConstant.SESSION_ATTR_USER_INFO) != null ? (UserInfo) getSessionAttribute(AppWebConstant.SESSION_ATTR_USER_INFO)
				: null;
		...
		try {
			...
			if (!isEmpty(uploadForm.getHotDealLink()))
				dealDTO.setHotDealLink(uploadForm.getHotDealLink());
			if (!isEmpty(uploadForm.getOrderNo()))
				dealDTO.setOrderNo(uploadForm.getOrderNo());

			userInfo.setCity(userInfo.getCityId());

			dealzDTO = getCityDealz.saveMerchantDealz(userInfo, dealDTO);
			if (!isEmpty(uploadForm.getFiles()) && !uploadForm.getFiles().get(0).isEmpty()) {
				if (documentDTO == null)
			...
			redirectAttributes.addFlashAttribute("message", "Deal Saved Successfully.");
		} catch (Exception e) {
			dealzDTO = new DealzDTO();
			e.printStackTrace();
			if (e instanceof BaseException) {
			...
			for (MultipartFile multipartFile : files) {
				...
				try {
					if (multipartFile.getSize() > 0) {
						...
						documentDTO.setFileSavedAs(DateUtil.getDateTimeStamp() + "_" + System.currentTimeMillis() + "_" + fileName);
						documentDTO.setOrignalFileName(fileName);
	...
	@RequestMapping("/component/openHotDeal")
	public ModelAndView openHotDealPage(ModelMap modelMap) {
		UserInfo userInfo = getSessionAttribute(AppWebConstant.SESSION_ATTR_USER_INFO) != null ? (UserInfo) getSessionAttribute(AppWebConstant.SESSION_ATTR_USER_INFO)
				: null;
		ModelAndView mav = new ModelAndView("admin/merchantManagement/addHotDeal");
		DealzDTO dealzDTO = null;
		try {
			Map<String, String> merchantsMap = getCityDealz.getAllUserList();
			Map<String, String> timeZoneMap = getCityDealz.getTimeZones();
			modelMap.put("timeZoneMap", timeZoneMap);
			if (getRequestParam("dealzId") != null && !getRequestParam("dealzId").trim().isEmpty()) {
				dealzDTO = merchantService.getDealzDetailById(userInfo, getRequestParam("dealzId"), getRequestParam("dealType"));
				...
				dealzDTO = new DealzDTO();
				String nexVal = getCityDealz.getNextOrderOfHotDeal();
				dealzDTO.setOrderNo(nexVal);
				mav.addObject("dealzDTO", dealzDTO);
			}
			CityDTO cityDtos = getCityDealz.getCityDetails(userInfo.getCityId());
			modelMap.put("zoneId", cityDtos.getTimeZoneId());
			modelMap.put("merchantsMap", merchantsMap);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return mav;
